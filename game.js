...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const playerImg = new Image(); playerImg.src = "assets/player/runner.png";
  const orbImg = new Image(); orbImg.src = "assets/orbs/network_orb.png";
  ...
  let username = "";
  let score = 0, phase = 1, nextPhase = 500, lives = 3;
  let player = { x: 100, y: 880, width: 64, height: 80, vy: 0, gravity: 1.5, jumpPower: -25 };
  let jumpCount = 0, boostActive = false, boostTimer = 0;
  let orbs = [], spikes = [], powerups = [], trail = [];
  let frame = 0;
  const groundY = 960;
  ...
  function spawnOrb() {
    orbs.push({ x: 1920, y: 600 + Math.random() * 200, r: 24 });
  }
  ...
  function spawnPowerup() {
    const list = ["shield", "slowmo", "heart"];
    const type = list[Math.floor(Math.random() * list.length)];
    const emoji = type === "shield" ? "🛡️" : type === "slowmo" ? "🐢" : "💙";
    powerups.push({ x: 1920, y: 600 + Math.random() * 200, type, emoji });
  }

  function drawPopup(text) {
    ctx.fillStyle = "#fff";
    ctx.font = "30px Raleway";
    ctx.fillText("✅ " + text, canvas.width / 2 - 150, 100);
  }
  ...
  function gameLoop() {
    ...
    powerups.forEach((p, i) => {
      ...
      if (player.x < p.x + 30 && player.x + player.width > p.x &&
        ...
        if (p.type === "shield") {
          boostActive = true; boostTimer = 100;
          drawPopup("Supercollateral Activated!");
        } else if (p.type === "slowmo") {
          drawPopup("CDP Protocol Engaged!");
        } else if (p.type === "heart" && lives < 3) {
          lives++;
          drawPopup("Heart Restored");
        }
      }
    });
    ...
    if (score >= nextPhase) {
      phase++;
      nextPhase += 500;
      drawPopup("Dynamic Repayment Vault Reached!");
    }
